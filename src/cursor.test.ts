...
describe("normalizeCursor", () => {
  ...
  describe("arbitrary-precision exactness", () => {
    ...
    it("deep round-trip: ascend 20 levels then re-descend", async () => {
      ...
      const result = await normalizeCursor(binary, state);
      await expectSameGlobal(binary, state, result);
    });

    it("deep round-trip: ascend 50 levels with asymmetric model", async () => {
      const depth = 50;
      const prefix = Array<string>(depth).fill("A");
      const state: Cursor<string> = {
        prefix,
      ...
      const result = await normalizeCursor(asym, state);
      await expectSameGlobal(asym, state, result);
    });

    it("deep descent lands exactly at zero", async () => {
      // x = 1 − 2^(−5) = 0.96875, y = 0 (stays 0 throughout).
      // After 5 descents into A: x should be exactly 0, y stays 0.
      const x = 1 - 2 ** -5;
      const r = await normalizeCursor(binary, { prefix: [], x, y: 0 });
      expect(r.prefix).toEqual(["A", "A", "A", "A", "A"]);
      expect(r.x).toBe(0); // exact zero, not ≈0
      expect(r.y).toBe(0);
    });

    it("cross-branch sibling transfer at depth 30", async () => {
    ...
    it("many-token distribution preserves position exactly", async () => {
      ...
      const result = await normalizeCursor(manyModel, state);
      await expectSameGlobal(manyModel, state, result);
    });
  });
});
