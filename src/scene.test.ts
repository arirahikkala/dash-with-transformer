...
function allNodes<T>(scene: Scene<T>): SceneNode<T>[] {
  const result: SceneNode<T>[] = [];
  function walk(nodes: SceneNode<T>[]) {
...
describe("buildScene", () => {
  ...
  describe("structure", () => {
    it("fully zoomed out shows all tokens at top level", () => {
    ...
    it("fully zoomed out with ternary model shows all three tokens", () => {
      const cursor: Cursor<string> = { prefix: [], x: 0, y: 0.5 };
      const scene = buildScene(ternary, cursor, 0.001);
      expect(topTokens(scene)).toEqual(["X", "Y", "Z"]);
    });

    it("zoomed in shows subset of tokens", () => {
      // Deep into A's square: only A and its children should be prominent
      const cursor: Cursor<string> = { prefix: ["A", "A", "A"], x: 0, y: 0.25 };
      const scene = buildScene(binary, cursor, 0.01);
    ...
    it("children are nested within parent bounds", () => {
      const cursor: Cursor<string> = { prefix: [], x: 0, y: 0.5 };
      const scene = buildScene(binary, cursor, 0.001);

      function checkBounds(nodes: SceneNode<string>[]) {
  ...
  describe("window bounds", () => {
    it("window within unit square needs no ascent", () => {
    ...
    it("window exceeding unit square still produces nodes", () => {
    ...
    it("ascent from depth when window exceeds current square", () => {
  ...
  describe("affine correctness", () => {
    it("fully zoomed out: top-level nodes span [0,1]", () => {
    ...
    it("ternary model: y positions reflect cumulative probabilities", () => {
      // X=0.2, Y=0.5, Z=0.3
    ...
    it("zoomed in: cursor centered means crosshairs at 0.5", () => {
  ...
  describe("size threshold", () => {
    it("large minHeight excludes small tokens", () => {
      // With a large minHeight, only the biggest tokens survive
      const cursor: Cursor<string> = { prefix: [], x: 0, y: 0.5 };
      buildScene(ternary, cursor, 0.3);
      ...
      const scene2 = buildScene(ternary, cursor, 0.2);
  ...
  describe("invariants", () => {
    ...
    it("no overlapping siblings", () => {
      const cursor: Cursor<string> = { prefix: [], x: 0, y: 0.5 };
      const scene = buildScene(ternary, cursor, 0.01);

      function checkSiblings(nodes: SceneNode<string>[]) {
        for (let i = 1; i < nodes.length; i++) {
          expect(nodes[i].y0).toBeGreaterThanOrEqual(nodes[i - 1].y1 - 1e-10);
        }
        for (const n of nodes) {
          checkSiblings(n.children);
        }
      }
      checkSiblings(scene.children);
    });

    it("all visible nodes at least partially in [0,1]", () => {
  ...
  describe("edge cases", () => {
    it("cursor.x = 0 gives fully zoomed out view", () => {
      const cursor: Cursor<string> = { prefix: [], x: 0, y: 0.5 };
      const scene = buildScene(binary, cursor, 0.01);
      expect(scene.children.length).toBe(2);
    });

    it("empty model produces empty scene", () => {
      const cursor: Cursor<string> = { prefix: [], x: 0, y: 0.5 };
    ...
    it("works with numeric token types", () => {
      ...
      const scene = buildScene(numModel, cursor, 0.01);
      expect(scene.children.length).toBe(2);
  ...
  describe("deep prefix ascent", () => {
    it("ascent from depth 30 preserves correct window positioning", () => {
      const depth = 30;
    ...
    it("ascent from depth 30 with asymmetric model", () => {
      const depth = 30;
      const prefix = Array<string>(depth).fill("A");
      const cursor: Cursor<string> = { prefix, x: 0, y: 0.5 };
      const scene = buildScene(asym, cursor, 0.01);
      const nodes = allNodes(scene);
      expect(nodes.length).toBeGreaterThan(0);
      for (const n of nodes) {
        expect(n.y1).toBeGreaterThan(n.y0);
    ...
    it("zoomed in deep: window stays inside current square", () => {
